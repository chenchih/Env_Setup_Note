...
### 2. git add: Staging your file

This will add file changes that are in your working directory to your index

**Syntax**: `git add <filename>` ,  you can use below option command:
...
### 4. git commit: save in local repository
...
### 5. git log : show commit history detail
...
- HEAD show last N commit

  > HEAD ~N #N last commit
  > git log --oneline -5


### 6. git push: push into remote repository
#### push to remote repository
This Git command will push all the modified local objects to the remote repository(github).
- go to github page and add a new respository
...
####  git push (local repository) to different remote repository
...
We can use the same local repository with multiply remote repository in github, but the remote repository name (origin) need to change.  

For example I have `test` in <u>remote repository(github)</u>, let create another <u>new remote repository</u> as `test1` and use the same <u>local repository</u>.
...
- `git clone https://github.com/<username>/test1.git test2` duplicate rep name test2
...
#### 8. git clean(before stage): same as delete file.
...
#### 9. git reset use when commit :
There are three type of reset: **hard, soft and mixed**.  We will often use this method to reset to previous commit
...
#### switch to remote branch from local
